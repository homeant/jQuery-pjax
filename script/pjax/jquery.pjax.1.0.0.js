 ...
 * git:https://git.oschina.net/dianyun/jQuery-pjax.git
 */
...
(function($) {
	...
	function fnPjax(selector, container, options) {
		...
		if(options.enableHash){
			if(options.defaultUrl && window.location.hash == ''){
				window.location.hash = options.defaultUrl;
			}
			var hash = $.trim(window.location.hash);
			if(hash){
				hash = hash.replace(/^(\#\!)?\#/, '');
				var ops = $.extend(true,{},options,{url:options.contentUrl(hash)});
				pjaxReload(container,ops);
			}
		}
	...
	function pjax(options) {
		...
		options.success = function(data, status, xhr) {
			...
			if (options.push || options.replace) {
				if(options.enableHash){
					window.history.replaceState(pjax.state, container.title, options.hash);
				}else{
	...
	function loadAjaxScript(scripts, callback) {
		...
		setTimeout(function() {
			...
			for (var i = 0; i < scripts.length; i++) {
				if (scripts[i]) {
					(function() {
						deferreds.push($.getScript(scripts[i]).done(function() {
	...
	function enable() {
		...
		$.pjax.loadAjaxScript = loadAjaxScript;
